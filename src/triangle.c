...
static inline void triangle_top(const point_t p[3]) {
	...
	const float y_diff0 = 1.f / (p[1].y - p[0].y);
	const float y_diff1 = 1.f / (p[2].y - p[0].y);
	const float x_inc0 = (p[1].x - p[0].x) * y_diff0;
	const float x_inc1 = (p[2].x - p[0].x) * y_diff1;
	const float z_inc0 = (p[1].z - p[0].z) * y_diff0;
	const float z_inc1 = (p[2].z - p[0].z) * y_diff1;
	const float r_inc0 = (p[1].r - p[0].r) * y_diff0;
	const float r_inc1 = (p[2].r - p[0].r) * y_diff1;
	const float g_inc0 = (p[1].g - p[0].g) * y_diff0;
	const float g_inc1 = (p[2].g - p[0].g) * y_diff1;
	const float b_inc0 = (p[1].b - p[0].b) * y_diff0;
	const float b_inc1 = (p[2].b - p[0].b) * y_diff1;
	const int yoff = abs(starty - (int)p[0].y);
	float x0 = p[0].x + x_inc0 * yoff; float x1 = p[0].x + x_inc1 * yoff;
	float z0 = p[0].z + z_inc0 * yoff; float z1 = p[0].z + z_inc1 * yoff;
	float r0 = p[0].r + r_inc0 * yoff; float r1 = p[0].r + r_inc1 * yoff;
	float g0 = p[0].g + g_inc0 * yoff; float g1 = p[0].g + g_inc1 * yoff;
	float b0 = p[0].b + b_inc0 * yoff; float b1 = p[0].b + b_inc1 * yoff;
	if (x_inc0 < x_inc1) {
...
static inline void triangle_bottom(const point_t p[3]) {
	int starty = (int)p[0].y < 0 ? 0 : ((int)p[0].y > YRES ? YRES : (int)p[0].y);
	...
	const float b_min = fmin(p[0].b, fmin(p[1].b, p[2].b));
	const float b_max = fmax(p[0].b, fmax(p[1].b, p[2].b));
	const float y_diff0 = 1.f / (p[2].y - p[0].y);
	const float y_diff1 = 1.f / (p[2].y - p[1].y);
	const float x_inc0 = (p[2].x - p[0].x) * y_diff0;
	const float x_inc1 = (p[2].x - p[1].x) * y_diff1;
	const float z_inc0 = (p[2].z - p[0].z) * y_diff0;
	const float z_inc1 = (p[2].z - p[1].z) * y_diff1;
	const float r_inc0 = (p[2].r - p[0].r) * y_diff0;
	const float r_inc1 = (p[2].r - p[1].r) * y_diff1;
	const float g_inc0 = (p[2].g - p[0].g) * y_diff0;
	const float g_inc1 = (p[2].g - p[1].g) * y_diff1;
	const float b_inc0 = (p[2].b - p[0].b) * y_diff0;
	const float b_inc1 = (p[2].b - p[1].b) * y_diff1;
	const int yoff = abs(endy - (int)p[2].y);
	float x0 = p[2].x - x_inc0 * yoff; float x1 = p[2].x - x_inc1 * yoff;
	float z0 = p[2].z - z_inc0 * yoff; float z1 = p[2].z - z_inc1 * yoff;
	float r0 = p[2].r - r_inc0 * yoff; float r1 = p[2].r - r_inc1 * yoff;
	float g0 = p[2].g - g_inc0 * yoff; float g1 = p[2].g - g_inc1 * yoff;
	float b0 = p[2].b - b_inc0 * yoff; float b1 = p[2].b - b_inc1 * yoff;
	if (x_inc0 >= x_inc1) {
