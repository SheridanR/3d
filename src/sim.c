...
void update() {
	while (frames_to_do) {
		...
		vec4_t right = quat_to_vec3(&camera.ang);
		vec4_t down = quat_to_vec3(mul_quat(&quat(), &camera.ang, &quat_copy(euler_to_quat(0.f, 0.f, (float)PI / 2.f))));
		vec4_t forward = quat_to_vec3(mul_quat(&quat(), &camera.ang, &quat_copy(euler_to_quat(0.f, (float)PI / 2.f, 0.f))));
